...
http.Response authErrorResponse() {
  return http.Response(jsonEncode({"message": encodeUtf8("احراز هویت ناموفق لطفا مجدد وارد حساب کاربری شوید"), "status": false}), 401, headers: {HttpHeaders.contentTypeHeader: 'application/json; charset=utf-8'});
}

http.Response timeoutErrorResponse() {
  return http.Response(jsonEncode({"message": encodeUtf8("خطا در دریافت اطلاعات مجدد تلاش کنید"), "status": false}), 408, headers: {HttpHeaders.contentTypeHeader: 'application/json; charset=utf-8'});
}

Future<http.Response> handleReq(Future<http.Response> req) async {
  var response = await req;
  if (LOG_RESPONSE) {
    print("REQUEST : " + response.request!.method.toString() + " " + response.request!.url.toString());
