...
class ConsultantFilesBloc extends Bloc<FilesEvent, FilesState> {
  ConsultantFilesBloc() : super(FilesInitState()) {
    on(_onEvent);
  }
  ...
  _onEvent(event, emit) async {
    if (event is FilesLoadEvent) {
      ...
      var url = getFileUrl('file/filesConsultant?consultantId=${event.filterData.consultantId}${event.filterData.toQueryString(delimiter: "&")}&lastId=${event.lastId}');
